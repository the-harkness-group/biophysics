...
# -*- coding: utf-8 -*-
"""
Created on Sat May  8 10:32:24 2021
...
def main():
    ...
    for peak in params['Single peaks']:
        ...
        for index, file in enumerate(params['Sparky files']):
            ...
            # Get 1D 1H trace along indirect dimension
            peakbox_1D_indirectppmindex = [(np.abs(indirect_ppm - indirect_coordinates)).argmin()]
            
#            print(peakbox_1D_indirectppmindex)
#            print(indirect_ppm[peakbox_1D_indirectppmindex])
            
            peakbox_1D_directppmindices = [(np.abs(direct_ppm - (direct_coordinates - params['Single peaks'][peak]['Direct pad'][index]))).argmin(),
            (np.abs(direct_ppm - (direct_coordinates + params['Single peaks'][peak]['Direct pad'][index]))).argmin()]
            
            peakbox_1D = data[peakbox_1D_indirectppmindex,
                peakbox_1D_directppmindices[-1]:peakbox_1D_directppmindices[0]+1]
...
#                           indirect_ppm[peakbox_indirectppmindices[-1]], indirect_ppm[peakbox_indirectppmindices[0]]))
            direct_1D_ppm = direct_ppm[peakbox_1D_directppmindices[-1]:peakbox_1D_directppmindices[0]+1]
            
            ax.plot(direct_1D_ppm,np.ravel(peakbox_1D),linewidth=2,color=basecmap[index]) # Direct lineshapes
...
#        
        ...
        # save the figure
        fig.tight_layout()
        fig.savefig(f"{peak}_1Dslice.pdf",format='pdf')
