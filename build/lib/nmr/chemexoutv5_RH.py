...
def makeChemEx(params, groups):
    ...
    os.makedirs(f"../chemex_analysis/Data/{str(params['B0']).split('.')[0]}") # Uses B0 field to make data directory for ChemEx
    ...
    experiment_file.write(f"#name = {str(params['B0']).split('.')[0]}_MHz\n")
    experiment_file.write(f"type = {params['Type']}\n\n")
    experiment_file.write('[experimental_parameters]\n')
    experiment_file.write(f"h_larmor_frq      = {params['B0']}\n")
    ...
    experiment_file.write('[extra_parameters]\n')
    experiment_file.write(f"path = ../Data/{str(params['B0']).split('.')[0]}/\n")
    ...
    for ind, group in groups:
        ...
        if  R2b_sort[1][0]-R2b_sort[1][len(vcpmg)-1]>threshold:
            ...
            np.savetxt(f"../chemex_analysis/Data/{str(params['B0']).split('.')[0]}/"+str(name)+".out",np.transpose(R2a))
            
            #c13_string = group.assignment.iloc[0].split('_')[2] + 'CD1 ' # Write to 13C shift file
            #c13_cs = group.center_y_ppm.iloc[0]
