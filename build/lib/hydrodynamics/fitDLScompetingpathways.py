...
# -*- coding: utf-8 -*-
"""
Created on Fri May 29 13:02:20 2020
...
def main():
    ...
    fit_params.add('dCpn', value=-1000)
    
    fit_params.add('dHe_0', value=50000)
    fit_params.add('Ke_0', value=20000)
    fit_params.add('dCpe', value=-1000)

    fit_params.add('T0', value=30+273.15,vary=False)
    
    fit_params.add('Rh_3', value=4.6965e-9, vary=True)
    fit_params.add('m',value=-0.333,vary=False)
    #fit_params.add('n',value=2,vary=False)
    fit_params.add('N',value=100,vary=False)
    fit_params.add('k_d',value=0)
    #fit_params.add('kd1',value=-43.4085)
    ...
    if run_fit == 'y':
        ...
        print('\n')
        fit_params.pretty_print(fmt='e',colwidth=12,columns=['value']) # Print nice paramater table
...
def supercageNucPol(fit_params, group_WT):
    ...
    for y in range(len(Temperatures)):
        ...
        constants = [(Concentrations[0]*Ke/3),Kn/Ke] # XT, alpha = Kn/Ke
        X_guess = 1e-5
        ...
        for x in range(1,N+1):
            ...
            if x == 2:
                ...
                c_dict[f"c1{3*x}"] = np.append(c_dict[f"c1{3*x}"],c3x) # Hexamer 1 dictated by trimer and Kn
                ...
                c_dict[f"c2{3*x}"] = np.append(c_dict[f"c2{3*x}"],c3x) # Hexamer 2 dictated by trimer and Ke
...
def ffs3(constants,X):
    ...
    XT, alpha = constants # Unpack constants
    
    eq1 = -XT + alpha*np.square(X) + (X/np.square(1. - X)) # Solution to competing pathways model
