...
def supercage96(fit_params, group_WT):
    ...
    for y in range(len(Temperatures)):
        ...
        kc_T = m*(Temperatures[y] - T0) + kc_0

        M3_num = (M3**2)*c3*(D3_0*(1 + kc_T*Concentrations[0]))
        M6_num = (M6**2)*c6*(D6_0*(1 + kc_T*Concentrations[0]))
        M12_num = (M12**2)*c12*(D12_0*(1 + kc_T*Concentrations[0]))
        M24_num = (M24**2)*c24*(D24_0*(1 + kc_T*Concentrations[0]))
        M48_num = (M48**2)*c48*(D48_0*(1 + kc_T*Concentrations[0]))
        M72_num = (M72**2)*c72*(D72_0*(1 + kc_T*Concentrations[0]))
        M96_num = (M96**2)*c96*(D96_0*(1 + kc_T*Concentrations[0]))
...
def main():
    ...
    fit_params.add('dCp6', value=-6000)
    ...
    fit_params.add('kc_0',value=-300)
    fit_params.add('m',value=50)
