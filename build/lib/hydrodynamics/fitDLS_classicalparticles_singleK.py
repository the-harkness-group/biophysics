...
# -*- coding: utf-8 -*-
"""
Created on Tue Apr 27 15:51:17 2021
...
import classicalparticles_singleK
...
def main():
    ...
    fit_params.add('dH1o', value=29,vary=True) # dH1 at ref temp.
    fit_params.add('K1o', value=56542,vary=True) # K1 at ref temp.
    fit_params.add('dCp1', value=-1.96,vary=True) # K1 dCp
    ...
    #state_list = ['6A','12B','24B']
    P_dict, C_dict = bindingmodels.make_dictionaries(state_list)
    D_dict = hydrodynamics.make_diffusion_dictionary(P_dict)
    
    #sim_func = classicalparticles_singleK.classical_3state
    sim_func = classicalparticles_singleK.classical_4state
...
#        pickle.dump(confidence_lnKs, fp, protocol=pickle.HIGHEST_PROTOCOL)
    ...
    for concentration in params['Simulation concentration']:
        for x, temperature in enumerate(params['Simulation temperature']):
            ...
            Psum = sum(P_dict.values())
            print('Population sum is:',Psum)
            
            for k in P_dict.keys():
            
                Pax[0].bar(int(k[:-1]),P_dict[k],color=hist_colors[x],edgecolor=hist_colors[x],width=1)
                Pax[1].bar(int(k[:-1]),P_dict[k],color=hist_colors[x],edgecolor=hist_colors[x],width=1)
