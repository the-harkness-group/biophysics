...
# -*- coding: utf-8 -*-
"""
Created on Tue Apr 27 10:09:23 2021
...
def main():
    ...
    fit_params.add('dH1o', value=4.990,vary=True) # dH1 at ref temp.
    fit_params.add('K1o', value=591542.997,vary=True) # K1 at ref temp.
    fit_params.add('dCp1', value=-5.1346,vary=True) # K1 dCp
    fit_params.add('dH2o', value=32.488,vary=True) # dH2 at ref temp.
    fit_params.add('K2o', value=27748.4996,vary=True) # K2 at ref temp.
    fit_params.add('dCp2', value=-6.6139,vary=True) # K2 dCp
    fit_params.add('dH3o', value=159.4797,vary=True) # dH2 at ref temp.
    fit_params.add('K3o', value=13109.249,vary=True) # K2 at ref temp.
    fit_params.add('dCp3', value=-0.9067,vary=True) # K2 dCp
    fit_constants = {'To':params['Fit params']['To']['Value']+273.15,'Rh_3':params['Fit params']['Rh_3']['Value'],
                     ...
                     params['Eta params'][3]],'N':params['Fit params']['N']['Value'],
                    'h_scale':params['Fit params']['h_scale']['Value'],'j_scale':params['Fit params']['j_scale']['Value']}
    ...
    if run_fit == 'y': # Fit data
        ...
        if get_errors == 'y':
            ...
            wrapper_args, observe, MC_iter, RMSD=np.sqrt(result.chisqr/result.ndata))
            time1 = time.time()
    ...
    if run_fit == 'n': # Simulate data, if optimal parameters already known
        if get_errors == 'y':
            resid = objective(fit_params, fit_data, wrapper_func, wrapper_args, observe) # For RMSD
            
            time0 = time.time()
            MC_dict, MC_errors = mc(fit_data, fit_params, fit_constants, wrapper_func,
            wrapper_args, observe, MC_iter, RMSD=np.sqrt(np.sum(np.square(resid))/len(fit_data)))            
            time1 = time.time()
            print(f"\nThe total elapsed Monte-Carlo error iteration time is {time1-time0}s\n")
...
def plot_fit(fit_data, opt_params, wrapper_func, wrapper_args, fit_constants, 
    ...
    D_ax2.set_ylabel('$D_{0}$ $cm^{2}$ $s^{-1}$')
    ...
    D_ax2.set_xticklabels(['$M_{6,expt.}$','$M_{6,calc.}$','$M_{3,expt.}$','$M_{3,calc.}$'],rotation=90)
...
#        K_ax.plot(1./(DLS_Temperature),np.log(K_dict['K1']),linewidth=2,color='#4393c3',label='$K_{1}$')
#        K_ax.plot(1./(DLS_Temperature),np.log(K_dict['K2']),linewidth=2,color='#b2182b',label='$K_{2}$')
#
...
#
#        K_ax.plot(1./(DLS_Temperature),lnK1_mean,linewidth=2,color='#4393c3',label='$K_{1}$')
#        K_ax.fill_between(1./(DLS_Temperature),lnK1_lower,lnK1_upper,facecolor='#4393c3',edgecolor='#4393c3',alpha=0.1)
#        K_ax.plot(1./(DLS_Temperature),lnK2_mean,linewidth=2,color='#b2182b',label='$K_{2}$')
...
#        pickle.dump(confidence_lnKs, fp, protocol=pickle.HIGHEST_PROTOCOL)
    ...
    for concentration in params['Simulation concentration']:
        for x, temperature in enumerate(params['Simulation temperature']):
            ...
            Psum = P_dict['3A'] + P_dict['6A'] + P_dict['12B'] + P_dict['24B']
            print('Population sum is:',Psum)
            
            Pax[0].bar(3,P_dict["3A"],color=hist_colors[x],edgecolor=hist_colors[x],width=1)
            Pax[1].bar(3,P_dict["3A"],color=hist_colors[x],edgecolor=hist_colors[x],width=1)
            Pax[0].bar(6,P_dict["6A"],color=hist_colors[x],edgecolor=hist_colors[x],width=1)
            Pax[1].bar(6,P_dict["6A"],color=hist_colors[x],edgecolor=hist_colors[x],width=1)
            Pax[0].bar(12,P_dict["12B"],color=hist_colors[x],edgecolor=hist_colors[x],width=1)
            Pax[1].bar(12,P_dict["12B"],color=hist_colors[x],edgecolor=hist_colors[x],width=1)
            Pax[0].bar(24,P_dict["24B"],color=hist_colors[x],edgecolor=hist_colors[x],width=1)
            Pax[1].bar(24,P_dict["24B"],color=hist_colors[x],edgecolor=hist_colors[x],width=1)
            
            Pax[0].text(0.9,0.6,'{}\n$M_{{T}}$ = {} $\mu$M\n{} \N{DEGREE SIGN}C'.format(params['Sample name'],concentration,temperature),
                 fontsize=24,va='top',ha='right',ma='left',transform=Pax[0].transAxes)
