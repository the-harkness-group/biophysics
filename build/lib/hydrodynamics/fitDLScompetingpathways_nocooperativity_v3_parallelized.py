...
def initialize_shared_memory(shared_perturbed_df_columns, \
    ...
    shared_pm_object, shared_pm_size, shared_opt_params_object, shared_opt_params_size):
    #
    ...
    #
    shared_plot_minimization['STRING'] = shared_pm_object
    shared_plot_minimization['SIZE'] = shared_pm_size
    shared_opt_params_str['STRING'] = shared_opt_params_object
    shared_opt_params_str['SIZE'] = shared_opt_params_size
#
# Task for parallel execution.
def lmFitTask(counter, RMSD):
    ...
    # plot_minimization - convert raw array to string.
    plot_minimizationS = np.frombuffer(shared_plot_minimization['STRING'], dtype = 'U1').reshape(shared_plot_minimization['SIZE'])
    plot_minimization = ''.join(map(str, plot_minimizationS))  # Convert np.array of characters back into string.
    ...
    perturbed_data = perfect_data_df.copy() # Copy perfect data groups
    perturbed_data.D = perturbed_data.D + np.random.RandomState().normal(scale=RMSD, size=np.size(perturbed_data.D)) # Perturb perfect data for MC analysis
...
def MonteCarloErrors(data, opt_params, fit_result, MC_iter):
    ...
    print(perfect_data_int64)
    print(perfect_data_int64.size)
    ...
    # plot_minimization.  (Could also have been simply passed in as a function argument, like RMSD.)
    plot_minimization_data = RawArray('u', len(plot_minimization))  # Shared array of utf-8.
    plot_minimization_strS = np.frombuffer(plot_minimization_data, dtype = 'U1').reshape(len(plot_minimization))
    np.copyto(plot_minimization_strS, list(plot_minimization))  # Use list() to preprocess string into characters for np.array.
    ...
    """
    # Original code (moved to lmFitTask()) and using shared memory.
    initialize_shared_memory(perfect_data.columns.tolist(), \
        ...
        plot_minimization_data, len(plot_minimization), \
        opt_params_data, len(opt_params_str))
    for counter in np.arange(1, MC_iter + 1):
        lmFitTask(counter, RMSD)
    """
    ...
    map_lmFitTask = partial(lmFitTask, RMSD = RMSD)
    ...
    for x in range(MC_iter):
        ...
        #print("plot_minimization",type(plot_minimization),len(plot_minimization))
        #print("opt_params",type(opt_params),len(opt_params.dumps()))
        perturbed_result = minimize(objective, opt_params, method='nelder', args=(plot_minimization, perturbed_groups))
        {MC_dict[k].append(perturbed_result.params[k]) for k in perturbed_result.params.keys()}
    ...
    with open(f"MC_parameter_dictionary_{MC_iter}iterations_v3parallelized.pickle",'wb') as f:
    ...
    for ind, group in fit_groups:
        ...
        if ind in pop_plot_concs:
            ...
            for x in range(len(hist_temperatures)):
                ...
                Pfigs[x][1][1].set_xticks([0, 10, 20, 30, 40])
                Pfigs[x][1][1].set_xticklabels([0, 10, 20, 30, 40])
