...
# -*- coding: utf-8 -*-
"""
Created on Fri Jan 10 12:39:42 2020
...
# -*- coding: utf-8 -*-
"""
Created on Thu Jan  9 14:55:57 2020
...
def supercage96(fit_params, group_WT):
    ...
    kc48 = fit_params['kc48'].value
    kc72 = fit_params['kc72'].value
    kc96 = fit_params['kc96'].value
    ...
    for y in range(len(Temperatures)):
        ...
        M48_num = (M48**2)*c48*(D48_0*(1 + kc48*Concentrations[0]))
        M72_num = (M72**2)*c72*(D72_0*(1 + kc72*Concentrations[0]))
        M96_num = (M96**2)*c96*(D96_0*(1 + kc96*Concentrations[0]))
...
def plot_fit(data_WT, opt_params):
    ...
    for ind, group in data_WT:
        ...
        D_all_ax.plot(sim_df['Temperature'],np.array(D_dict['D48_0'])*1e4,color=colors[79],label='$D_{48,0}$')
        D_all_ax.plot(sim_df['Temperature'],np.array(D_dict['D72_0'])*1e4,color=colors[39],label='$D_{72,0}$')
        D_all_ax.plot(sim_df['Temperature'],np.array(D_dict['D96_0'])*1e4,color=colors[20],label='$D_{96,0}$')
        D_all_ax.plot(group.Temperature,group.D*1e4,'ko',label='Experiment')
        D_all_ax.plot(sim_df['Temperature'],np.array(D_dict['Dz'])*1e4,'r',label='Fit')
...
def main():
    ...
    data_WT = data_WT[data_WT['Concentration'].isin([862.7, 328.8, 159.5, 77.4, 37.5, 18.2, 8.8])]
    ...
    fit_params.add('dCp6', value=-10000)
    
    fit_params.add('dH12_0', value=216153)
    fit_params.add('K12_0', value=18779.7627)
    fit_params.add('dCp12', value=-4000)
    
    fit_params.add('dH24_0', value=137019.415)
    fit_params.add('K24_0', value=20932.7973)
    fit_params.add('dCp24', value=-4000)

    fit_params.add('dH48_0', value=100000)
    fit_params.add('K48_0', value=11646.2756)
    fit_params.add('dCp48', value=-4000)
    
    fit_params.add('dH72_0', value=144303.865)
    fit_params.add('K72_0', value=6121.53652)
    fit_params.add('dCp72', value=-4000)

    fit_params.add('dH96_0', value=293047.933)
    fit_params.add('K96_0', value=15109.2265)
    fit_params.add('dCp96', value=-4000)
    
    fit_params.add('Rh_3', value=5.0e-9,vary=False)
    fit_params.add('n',value=-0.333,vary=False)
    #fit_params.add('k_c', value=-52.56)
    fit_params.add('kc3',value=-10)
    fit_params.add('kc6',value=-10)
    fit_params.add('kc12',value=-10)
    fit_params.add('kc24',value=-100)
    fit_params.add('kc48',value=-200)
    fit_params.add('kc72',value=-500)
    fit_params.add('kc96',value=-500)
