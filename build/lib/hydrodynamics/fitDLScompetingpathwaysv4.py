...
def supercageNucPol(fit_params, fit_group):
    ...
    K_dict = {'Kn1':[None]*len(Temperatures),'Kn2':[None]*len(Temperatures),'Ke':[None]*len(Temperatures)}
    
    # Calculate equilibrium constants
    Kn1 = Kn1_0*np.exp((dHn1_0/R)*((1/T0) - (1/Temperatures)) + (dCpn1/R)*(np.log(Temperatures/T0) + (T0/Temperatures) - 1))
    Kn2 = Kn2_0*np.exp((dHn2_0/R)*((1/T0) - (1/Temperatures)) + (dCpn2/R)*(np.log(Temperatures/T0) + (T0/Temperatures) - 1))
    Ke = Ke_0*np.exp((dHe_0/R)*((1/T0) - (1/Temperatures)) + (dCpe/R)*(np.log(Temperatures/T0) + (T0/Temperatures) - 1))
    ...
    for index, Temperature in enumerate(Temperatures):
        ...
        constants = [(Concentrations[0]*Ke[index]/3),Kn1[index]/Kn2[index],Kn2[index]/Ke[index],n] # XT, alpha1 = Kn1/Kn2, alpha2 = Kn2/Ke, nucleus size
        ffs_partial = partial(ffs3,constants)  
        sol = opt.root(ffs_partial,X_guess,method='lm')         
        c3 = sol.x[0]/Ke[index] # Trimer concentration in molar        
        ...
        for x in range(1,N+1):
            ...
            if 1 < x <= n:
                ...
                if x == 2:
                    ...
                    c3x = Kn1[index]*c_dict[f"c{3*(x-1)}"][index]*c3
                    c_dict[f"c1{3*x}"][index] = c3x # Hexamer 1 dictated by trimer and Kn1
                    P_dict[f"P1{3*x}"][index] = 3*x*c3x/Concentrations[0] # Hexamer 1 dictated by trimer and Kn1
                    ...
                    # Hexamer 2
                    D3x_0 = D3_0[index]*x**(j_scale)
                    c3x = Kn2[index]*c_dict[f"c{3*(x-1)}"][index]*c3
                    c_dict[f"c2{3*x}"][index] = c3x # Hexamer 2 dictated by trimer and Kn2
                    P_dict[f"P2{3*x}"][index] = 3*x*c3x/Concentrations[0] # Hexamer 2 dictated by trimer and Kn2
                ...
                if x == 3:
                    c3x = Kn2[index]*c_dict[f"c2{3*(x-1)}"][index]*c3
                    c_dict[f"c{3*x}"][index] = c3x # 9-mer dictated by hexamer 2 and Kn
                    P_dict[f"P{3*x}"][index] = 3*x*c3x/Concentrations[0]
                ...
                if x > 3:
                    c3x = Kn2[index]*c_dict[f"c{3*(x-1)}"][index]*c3
                    c_dict[f"c{3*x}"][index] = c3x # >9-mer dictated by 9-mer and Kn
                    P_dict[f"P{3*x}"][index] = 3*x*c3x/Concentrations[0]
            ...
            if x > n:
                ...
                if x == 3:
                    c3x = Ke[index]*c_dict[f"c2{3*(x-1)}"][index]*c3
                    c_dict[f"c{3*x}"][index] = c3x # 9-mer dictated by hexamer 2 and Ke
                    P_dict[f"P{3*x}"][index] = 3*x*c3x/Concentrations[0]
                ...
                if x > 3:
                    c3x = Ke[index]*c_dict[f"c{3*(x-1)}"][index]*c3
                    c_dict[f"c{3*x}"][index] = c3x # >9-mer dictated by 9-mer and Ke
                    P_dict[f"P{3*x}"][index] = 3*x*c3x/Concentrations[0]
        ...
        X_guess = c3*Ke[index] # Solver guess for next temperature is solution at the i-1 temperature for stability
...
def objective(fit_params, plot_minimization, fit_groups):
    ...
    if plot_minimization == 'n': # Don't plot fit in real time
        ...
        for ind, group in fit_groups:
            ...
            resid.append(np.array(group.D.values*1e4) - np.array(D_dict['Dz'])*1e4)
    ...
    #time1 = time.time()
    #print(f"The elapsed time is {time1-time0} s")
