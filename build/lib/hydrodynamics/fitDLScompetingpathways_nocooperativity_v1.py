...
def main():
    ...
    pop_plot_concs = params['Plot concentrations']
    ...
    if run_fit == 'y': # Fit data
        ...
        if get_errors == 'y':
            ...
            plot_fit(fit_groups, opt_params, PDFname, samplename, figure_dir, MC_dict, pop_plot_concs)
    ...
    if run_fit == 'n': # Simulate data
        ...
        plot_fit(fit_groups, fit_params, PDFname, samplename, figure_dir, MC_dict, pop_plot_concs)
...
def MonteCarloErrors(data, opt_params, fit_result, MC_iter):
    ...
    for ind, group in perfect_groups: # Overwrite diffusion constants to be perfect simulated values from best fit params
        ...
        perfect_data['D'].loc[perfect_data['Concentration'] == group.Concentration.iloc[0]] = D_dict['Dz']
    ...
    for x in range(MC_iter):
        ...
        #plt.plot(data.Temperature,data.D,'ko')
        #plt.plot(perfect_data.Temperature,perfect_data.D,'bo')
        #plt.plot(perturbed_data.Temperature,perturbed_data.D,'ro')
...
def plot_fit(fit_groups, opt_params, PDFname, samplename, figure_dir, MC_dict, pop_plot_concs):
    ...
    for ind, group in fit_groups:
        ...
        if ind in pop_plot_concs:
            ...
            for x in range(len(hist_temperatures)):
                ...
                Pfigs.append(plt.subplots(2, 1, sharex=True, gridspec_kw={'height_ratios': [1,3]}))
                ...
                #Pfigs[x][1][0].set_xlabel("Oligomer size")
                Pfigs[x][1][0].set_xlim([0,41])
                #Pfigs[x][1][0].set_ylabel('Population')                
            ...
            for hcindex, temperature in enumerate(hist_temperatures):
                ...
                #Phist_figs[hcindex].savefig(f"{figure_dir}/Populations_{temperature}C_{group.Concentration.iloc[0]}uM.png")
    ...
    # Simulate populations at desired biological concentration of 192 uM
    sim_df = pd.DataFrame()
    sim_df['Temperature'] = np.linspace(5,50,100)
    sim_df['Concentration'] = np.zeros(len(sim_df['Temperature']))
    sim_df['Concentration'] = 192 # 192 uM
    D_dict, c_dict, P_dict = supercageNucPol(opt_params, sim_df)
