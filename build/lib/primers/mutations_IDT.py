...
# -*- coding: utf-8 -*-
"""
Created on Wed Jun  2 15:57:16 2021
...
# Make list for writing excel file later
excel_rows = []
...
# and reverse mutagenesis primers
    fivep_seq = ''.join([seq_dict[int(mut_index)-i][0] for i in reversed(range(1,10))])
    fivep_seq = fivep_seq.upper() # ENSURE THAT SEQUENCE IS ALL UPPERCASE FOR TM CALCULATION!
    fivep_Tm = calculate_Tm(fivep_seq)
    while fivep_Tm > 58:
        fivep_seq = fivep_seq[1:]
        fivep_Tm = calculate_Tm(fivep_seq)

    threep_seq = ''.join([seq_dict[int(mut_index)+i][0] for i in range(1,10)])
    threep_seq = threep_seq.upper() # ENSURE THAT SEQUENCE IS ALL UPPERCASE FOR TM CALCULATION!
...
# Get mutation without underscores for writing dictionary
    ...
    excel_rows.append([f"{seq_file.split('_')[0]}_{concise_mutation}_F",sense_primer,'25nm','STD'])
    excel_rows.append([f"{seq_file.split('_')[0]}_{concise_mutation}_R",anti_sense_primer,'25nm','STD'])

# Write .xlsx file for IDT since this is what their bulk input interface accepts
df = pd.DataFrame(excel_rows, columns=['Name','Sequence','Scale','Purification'])
df.to_excel(mutation_parameters['Excel name'],index=False)